...
| 항 | 수식 | 비유 |
|------|---------|---------------|
| **P** (비례) | $K_p \cdot e$ | 목표로 조향 |
| **I** (적분) | $\frac{K_p}{T_i} \int e \, dt$ | "너무 오래 벗어나 있었다" |
| **D** (미분) | $K_p T_d \frac{de}{dt}$ | 도착 전 브레이크 |
...
제어기가 $C^*$라면 $u = C^* \cdot e_v$가 정확히 성립. 따라서 최소화:
...
엄밀한 증명은 $N \to \infty$에서 $\arg\min J(\theta) \to C^*$를 보이는 것. Campi et al. (2002) Theorem 1 참조.

### 4.6 PID의 경우 — 선형 회귀

PID 형태 $C = \rho_1 + \rho_2/s + \rho_3 s$ 대입:
...
- $\kappa$ 작음 (< $10^3$): 데이터가 모든 방향을 잘 커버 → 좋은 회귀
- $\kappa$ 큼 (> $10^6$): 데이터가 한 방향으로 거의 평평 → P, I, D 기여를 구분 불가 → 신뢰 불가
...
길이 $N_{FFT} = 2^{\lceil \log_2(2N) \rceil}$이 되도록 0을 추가.
...
1. $M(j\omega_k)$, $W(j\omega_k)$, $F(j\omega_k) = M(1-M)W$ 계산
2. 가상 레퍼런스: $R_v = Y / M$
3. 가상 에러: $E_v = R_v - Y = (1/M - 1) \cdot Y$
4. 양측 필터링: $U_F = F \cdot U$, $E_F = F \cdot E_v$
...
9. **추출:** $K_p = \rho_1$, $T_i = K_p / a_I$, $T_d = a_D / K_p$
...
루프: $(SP \pm h) \to K_p \to \mathrm{Plant} \to y$

### 9.3 릴레이 작동

구형파 $\pm h$가 설정값에 추가되며, $y$가 중심을 교차할 때 전환:
...
릴레이가 $K_{p,old} \cdot P$를 보므로 (단독 $P$가 아님), 측정된 궁극 게인은 결합 시스템에 대한 것:

$$K_u^{meas} = \frac{4h}{\pi a}$$

플랜트의 진짜 궁극 게인 ($K_{p,old}$ 보정):
...
시작: $h = \max(2.0, 3 \times \mathrm{std\_{natural}})$. 5초 후 교차 2회 미만이면 $h$를 2배 (최대 10.0).
...
Wiener 정규화로 $H(j\omega) = Y/U$ 추정 후, 저주파 위상에 직선 피팅:

$$\tau = -\frac{\sum w_k \omega_k \phi_k}{\sum w_k \omega_k^2}$$

**위상 언래핑:** `atan2`는 $(-\pi, \pi]$ 반환. 큰 지연에서 위상이 감김. 인접 빈 간 $\pi$ 넘는 점프 감지 시 $\pm 2\pi$ 보정.
...
| 가상 레퍼런스 $r_v = M^{-1}y$ | ✅ | Core VRFT |
| OLS 회귀 | ✅ | Core VRFT |
| $T_i$ 대체 $4\tau + T_s$ | ⚠️ | 경험적 규칙 |
| 릴레이 피드백 (P-only + $K_u$ 보정) | ❌ | Åström-Hägglund 1984, 수정 |
| $\tau$ 위상 기울기 + 언래핑 | ❌ | 표준 시스템 식별 + 신호처리 |
