...
### 9.5 Tyreus-Luyben 규칙

새 PID 파라미터 (**`Kp_new`**, `Kp_old`와 혼동 주의):

$$K_{p,new} = 0.45 \, K_u^{plant}, \quad T_i = 2.2 \, T_u, \quad T_d = T_u / 6.3$$

Ziegler-Nichols (`Kp=0.6Ku, Ti=Tu/2`)보다 보수적. 오버슈트가 적고 산업에서 더 널리 사용됨.
...
| 릴레이 피드백 (P-only + `K_u` 보정, Tyreus-Luyben) | ❌ | Åström-Hägglund 1984 + Tyreus-Luyben |
| `τ` = dt 고정 | ❌ | FTD 물리 틱 기반 |
