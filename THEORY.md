...
이 모드는 세 가지 PID 튜닝 방법을 제공합니다:
1. **N4SID 서브스페이스 식별 (주방법)** — 폐루프 데이터에서 상태공간 모델 `(A,B,C,D)` 추출 후 PSO로 PID 탐색. 이론적으로 가장 강건.
2. **VRFT (대체)** — N4SID 실패 시 fallback. 모델 없이 회귀 한 번으로 PID 산출.
3. **OpenLoop Step ID (선택)** — 개루프 스텝 응답으로 FOPDT/IPDT 모델 + IMC-PID. 안전 비행 중 별도 사용.

**사용 흐름:**
```
[Auto Tune] → 가진 데이터 수집 → N4SID (실패 시 VRFT) → PSO 최적화 → 적용
```
...
| 한계 | 원인 | 완화 |
|-----------|-------|------------|
| N4SID 차수 과대평가 | 노이즈 → 작은 σ 살아남음 | Gavish-Donoho 임계값 + 차수 4 캡 |
| N4SID 가진 부족 시 실패 | `U_f U_f^T` 특이 | VRFT fallback |
| VRFT `T_i` 부정확 | `F(0) = 0` (구조적) | 수동 조정 또는 N4SID 사용 |
