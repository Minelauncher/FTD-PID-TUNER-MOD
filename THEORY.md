...
**QR 분해**는 이걸 수치적으로 안정적으로 계산하는 방법. `(ΦᵀΦ)⁻¹`을 직접 계산하면 조건이 나쁜 행렬에서 부정확.
...
`σ` 값은 `Φ`의 **SVD**에서 나오며, 데이터가 각 방향으로 얼마나 "펼쳐져" 있는지를 나타냄.

- `κ` 작음 (< 10³): 데이터가 모든 방향을 잘 커버 → 좋은 회귀
- `κ` 큼 (> 10⁶): 데이터가 한 방향으로 거의 평평 → P, I, D 기여를 구분 불가 → 신뢰 불가

`κ`가 크면 **가진이 부족** — 충분히 다양한 주파수로 흔들지 않았다는 뜻.
