...
**폴백:** `det(O) < 10⁻⁸` 또는 `n ≠ 2` → 완전 파라미터화 `vec(A), vec(B), vec(C), D, vec(K)` 사용. LM `λ·diag(J^TJ)` 정규화가 중복 흡수.
...
기존: 신호 약할 때 진폭 올리기만. 개선: **올리기 + 내리기** 양방향.
...
1. 5초간 전 축 `y` 수집 (가진 없음, 자연 변동만)
2. 축별 `std(y)` 계산
3. 중간값 대비 2배 초과 축 → **"HIGH"** 경고 (커플링 악화 가능성)
4. 결과: `"Axis 1: 0.3, Axis 2: 0.5, Axis 3: 2.1 HIGH"`
...
| **관측 정준형 (n=2)** | ✅ | 최소 파라미터화 표준 (Ljung 1999) |
