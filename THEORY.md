...
이 모드는 계층화된 PID 튜닝 파이프라인을 제공합니다:

1. **PEM (주방법)** — PBSID-opt + Gauss-Newton. 폐루프 **무편향**(consistency) 보장 + **Cramér-Rao bound** 점근적 도달. 혁신 형식 상태공간 `(A,B,C,D,K)` 추정.
2. **N4SID (2차 폴백)** — Van Overschee 표준. 경사투영 기반. 폐루프에선 편향 있음.
3. **VRFT (3차 폴백)** — 모델 없이 회귀 한 번. 초저비용 안전망.
4. **OpenLoop Step ID (선택)** — PID 우회 개루프 스텝. 안전 호버에서만.

**식별 체인:**
```
[Auto Tune] → 가진 데이터 수집 → PEM → (실패 시) N4SID → (실패 시) VRFT → PSO → 적용
```
...
## 9. N4SID — 서브스페이스 상태공간 식별

### 9.1 왜 N4SID?

VRFT는 회귀 한 번으로 끝나지만 두 가지 한계가 있다:
- **`F(0)=0` 구조적 결함** → `T_i` 추정 불안정 (§8)
- **모델이 없음** → 폐루프 응답 시뮬레이션 불가, 보수적 PID 외 보장 어려움

N4SID는 데이터에서 **이산시간 상태공간 `(A, B, C, D)`** 를 직접 추출. 모델이 있으면:
- 임의 PID에 대해 **폐루프 응답 시뮬** 가능
- ITAE/오버슈트 등 **명시적 비용** 으로 PSO 탐색
- DC 한계 없이 `K_p, T_i, T_d` 동시 최적화

이론적 근거: Van Overschee & De Moor, *"N4SID: Subspace algorithms for the identification of combined deterministic-stochastic systems"* (Automatica 1994).

### 9.2 데이터 모델

플랜트를 다음 LTI 이산 상태공간으로 가정:

$$x(k+1) = A x(k) + B u(k), \qquad y(k) = C x(k) + D u(k)$$

- `x ∈ ℝⁿ`: 내부 상태 (관측 불가, n은 알려지지 않음)
- `u ∈ ℝ`: 입력 (자기축 PID 출력, SISO)
- `y ∈ ℝ`: 출력 (PV)

목표: 측정 데이터 `{u(0..N-1), y(0..N-1)}` 만으로 `n` 과 `(A,B,C,D)` 추정.

### 9.3 블록 Hankel 행렬

과거/미래 horizon `i = min(20, ⌊(N-1)/4⌋)`, 열 수 `j = N - 2i + 1`.
4개의 `i × j` Hankel 행렬:

$$U_p = \begin{bmatrix} u(0) & u(1) & \cdots & u(j-1) \\ u(1) & u(2) & \cdots & u(j) \\ \vdots & & & \vdots \\ u(i-1) & u(i) & \cdots & u(i+j-2) \end{bmatrix}, \quad U_f = \begin{bmatrix} u(i) & \cdots & u(i+j-1) \\ \vdots & & \vdots \\ u(2i-1) & \cdots & u(2i+j-2) \end{bmatrix}$$

`Y_p, Y_f` 도 동일 방식. 각 열은 *길이 `i` 의 윈도우*; 열 인덱스 `c`가 *시간 시작점* 을 나타냄.

과거 입출력 결합:

$$W_p = \begin{bmatrix} U_p \\ Y_p \end{bmatrix} \in \mathbb{R}^{2i \times j}$$

`W_p` 는 시점 `i` 의 미지 상태 `x(i + c)` 를 결정하기에 충분한 정보를 담음 (관측 가능성 가정 하).

### 9.4 경사 투영 (Oblique Projection)

핵심 통찰: 결정론적 시스템이라면 `Y_f` 는 `(W_p, U_f)` 두 부분공간의 합으로 분해됨.

$$Y_f = \underbrace{\Gamma_i \cdot X_i}_{\text{상태로부터}} + \underbrace{H_i \cdot U_f}_{\text{미래 입력 직접 효과}}$$

여기서 `Γ_i = [C; CA; CA²; ...; CA^{i-1}]` 이 **확장 관측성 행렬** , `X_i = [x(i), x(i+1), ..., x(i+j-1)]` 가 상태 시퀀스, `H_i` 가 입력→출력 임펄스 응답을 모은 토플리츠 행렬.

상태 항만 추출하려면 `U_f` 방향의 성분을 제거해야 함. **경사 투영**:

$$\mathcal{O}_i = Y_f \, /_{U_f} \, W_p$$

= "`U_f` 의 행공간을 따라 `W_p` 의 행공간으로 `Y_f` 를 투영". 결과:

$$\mathcal{O}_i = \Gamma_i \cdot X_i \in \mathbb{R}^{i \times j}$$

(미래 입력 효과가 완전히 분리됨.)

**계산 공식:**

$$\Pi_{U_f^\perp} = I - U_f^T (U_f U_f^T)^{-1} U_f \quad (\text{U}_f\text{ 직교여집합 투영})$$

$$\mathcal{O}_i = Y_f \cdot \Pi_{U_f^\perp} \cdot \left( W_p \cdot \Pi_{U_f^\perp} \right)^\dagger \cdot W_p$$

(wide matrix `X` 의 의사역 `X^† = X^T (X X^T)^{-1}`.)

### 9.5 SVD + Gavish-Donoho 차수 결정

`O_i` 는 이론상 rank `n` (= 시스템 차수). SVD로 분해:

$$\mathcal{O}_i = U \Sigma V^T = \begin{bmatrix} U_n & U_\bot \end{bmatrix} \begin{bmatrix} \Sigma_n & 0 \\ 0 & \Sigma_\bot \end{bmatrix} V^T$$

노이즈가 있으면 `Σ_⊥ ≠ 0` 이라 임계값이 필요. **Gavish-Donoho 2014 최적 임계값** (unknown noise level):

$$\beta = \frac{\min(m, n)}{\max(m, n)}, \qquad \omega(\beta) = \sqrt{2(\beta+1) + \frac{8\beta}{(\beta+1) + \sqrt{\beta^2 + 14\beta + 1}}}$$

$$\tau = \omega(\beta) \cdot \sigma_{\text{median}}$$

`σ_i > τ` 인 특이값만 유지. 정방(`β=1`)일 때 `ω(1) = 4/√3 ≈ 2.309` 로 잘 알려진 값. 직사각이면 더 작은 ω가 적용되어 보수적이지 않음.

차수 상한은 4로 캡 (PID 계산 비용/과적합 방지).

### 9.6 상태 시퀀스 X_i, X_{i+1}

확장 관측성 행렬:

$$\Gamma_i = U_n \cdot \Sigma_n^{1/2} \in \mathbb{R}^{i \times n}$$

`O_i = Γ_i · X_i` 에서 의사역으로 상태 시퀀스 추출:

$$X_i = \Gamma_i^\dagger \cdot \mathcal{O}_i \in \mathbb{R}^{n \times j}$$

$X_i$ 의 열 $c$ 는 시점 $i + c$ 의 상태 추정.

`A, B, C, D` LS를 위해 **다음 시점의 상태** $X_{i+1}$ 도 필요. 이것은 *이동(shifted) 경사 투영* 으로 얻음:

- $Y_f^- = Y_f$ 의 첫 블록행 제거 ((i-1) × j)
- $U_f^- = U_f$ 의 첫 블록행 제거
- $W_p^+ = [U_p; \, U_f(1,:); \, Y_p; \, Y_f(1,:)]$ — 과거에 시점 `i` 한 블록 추가

$$\mathcal{O}_{i-1} = Y_f^- \, /_{U_f^-} \, W_p^+, \qquad \Gamma_{i-1} = \Gamma_i \text{의 마지막 블록행 제거}$$

$$X_{i+1} = \Gamma_{i-1}^\dagger \cdot \mathcal{O}_{i-1}$$

**왜 이동 투영이 필요한가?** 단순히 `X_i` 의 열을 한 칸 옮기면 (`X_i[:, 1:]`) 같은 윈도우의 서로 다른 시점 상태가 섞임. 이동 투영은 *시작점이 한 칸 미래인 새 윈도우* 의 상태를 독립적으로 재구성.
...
여기서 `Y_{i|i} = Y_f(0, :)` (시점 `i` 의 출력 1 × j), `U_{i|i} = U_f(0, :)` (시점 `i` 의 입력).

`P = [X_i; U_{i|i}]`, `Q = [X_{i+1}; Y_{i|i}]` 으로 두면 정규방정식:

$$\Theta = Q P^T (P P^T)^{-1}, \qquad \Theta = \begin{bmatrix} A & B \\ C & D \end{bmatrix} \in \mathbb{R}^{(n+1) \times (n+1)}$$

블록 분리로 `A` (n×n), `B` (n×1), `C` (1×n), `D` (스칼라) 추출.

### 9.8 모델 분석

**고유값 → 지배 시상수:** `A` 의 고유값 `λ_k` 로부터

$$\tau_k = -\frac{\Delta t}{\ln |\lambda_k|}, \quad \tau_{\text{dominant}} = \max_k \tau_k \quad (|\lambda_k| < 1)$$

**DC 이득:** `z = 1` 에서 전달함수

$$G(1) = C (I - A)^{-1} B + D$$

`(I-A)` 가 특이면 적분 모드 → DC = ∞ 로 표시.

### 9.9 폐루프 시뮬 — D 항과 algebraic loop

PID 제어:

$$u(k) = K_p \left[ e(k) + \frac{1}{T_i} \sum_{j=0}^{k} e(j) \Delta t + T_d \frac{e(k) - e(k-1)}{\Delta t} \right]$$

문제: `y(k) = C x(k) + D u(k)` 이고 `e(k) = r - y(k)` 이므로 `D ≠ 0` 일 때 `u(k)` 와 `y(k)` 가 서로 의존 → **algebraic loop**.

**해결: 1-샘플 지연 측정** ($y_{meas}(k) = y(k-1)$). 실제 디지털 제어기는 항상 ADC + 계산 시간만큼 지연되므로 물리적으로 정확. 추가 이점: `D` 항이 자연스럽게 다음 스텝의 상태에 전파됨, 별도 처리 불필요.
...
for k = 0 .. simLen-1:
    ...
    integ += e · dt
    deriv = (e - prevE) / dt
    u   = clip( Kp · (e + integ/Ti + Td·deriv),  -1, 1 )
    y   = C · x + D · u
    x   = A · x + B · u
    y_meas = y      # 다음 스텝에서 사용
```
...
탐색 변수: `(K_p, T_i, T_d)` , **로그 공간** 에서 균등 샘플링 (게인이 자릿수 단위로 변하므로).

- 입자 수: 20, 반복: 30
- 관성 `w = 0.7`, 인지 `c1 = 1.5`, 사회 `c2 = 1.5`
- 경계: `Kp ∈ [0.001, 1.0]`, `Ti ∈ [0.1, 50]`, `Td ∈ [0.001, 1.0]`
- 시드 = 42 (재현성)

비용: **ITAE + 오버슈트 페널티**

$$J = \sum_{k} k \cdot \Delta t \cdot |y_{ref}(k) - y(k)| \cdot \Delta t \;+\; 10 \cdot \max(0, \, \max_k y(k) - 1)$$

`y_ref` 는 2차 참조모델 (시상수 `τ_M = 0.4 τ_{dom}`) 의 스텝 응답. 시뮬 길이 = `min(800, max(200, 6 τ_{dom} / dt))`.

### 9.11 로버스트성 체크

| 단계 | 실패 모드 | 처리 |
|------|----------|------|
| Hankel 구성 | `j < 2i + 10` | 데이터 부족 예외, VRFT fallback |
| `(U_f U_f^T)^{-1}` | 입력 가진 부족 | LU 분해 실패 → fallback |
| `(W_p · Π · W_p^T)^{-1}` | 과거가 정보 부족 | 동일 |
| `Γ^T Γ` 역행렬 | 차수 과대평가 | 동일 |
| `(I-A)^{-1}` | 적분 모드 | DC = NaN 표시, 시뮬은 진행 |
| PSO 발산 | `|y| > 100` | 비용 = `MaxValue` |
...
| 한계 | 원인 | 완화 |
|-----------|-------|------------|
| **PEM local minima** | 비볼록 비용 `V(θ)` | PBSID-opt 초기화로 전역 근처 시작, LM 정규화 |
| **PEM 파라미터 중복** | 완전 파라미터화 → `n²`개 중복 (similarity) | LM `λI` 가 정칙화; 최적값 자체는 unique (동일 입출력) |
| PEM 불안정 수렴 | `A_K` 고유값 ≥ 1 | step halving + 안정성 제약 거부 |
| N4SID 폐루프 편향 | 경사투영 개루프 가정 | PEM 주방법, N4SID는 폴백 |
| 차수 과대평가 | 노이즈 → 작은 σ 살아남음 | Gavish-Donoho 임계값 + 차수 4 캡 |
| 가진 부족 → LS 특이 | 지속 가진(PE) 조건 미충족 | 멀티사인 진폭/대역폭 ↑, 적응형 진폭 |
| VRFT `T_i` 부정확 | `F(0) = 0` (구조적) | 수동 조정 또는 PEM/N4SID 사용 |
| 기동 시 보수적 `K_p` | 선형 가정 위반 | 안정 비행 중 튜닝 |
| 교차축 커플링 | SISO 모델 | 순차 튜닝 (Roll→Pitch→Yaw) |
| PSO 수렴 변동 | 확률적 탐색 | 시드 고정, 충분 반복 (30) |
| `D ≠ 0` 모델 시뮬 | algebraic loop | 1-샘플 지연 측정 (물리적으로 정확) |
...
| **N4SID 경사 투영** | ✅ | Van Overschee & De Moor 1994 |
| **이동(shifted) 투영 → X_{i+1}** | ✅ | Van Overschee Algorithm 1 |
| **Gavish-Donoho `ω(β)` 임계값** | ✅ | Gavish & Donoho 2014 |
| **상태공간 `[A B; C D]` LS** | ✅ | Subspace ID 표준 |
| **PSO (로그 공간)** | ✅ | Kennedy & Eberhart 1995 |
